...
SSH into the cluster first (`./access_cluster.py -N <cluster_name>` -- connects to the login node by default when one is enabled, the head node otherwise; pass `-H` to reach the head node directly). Both share the same NFS-mounted `$HOME`, so the path below is reachable either way. Then:
