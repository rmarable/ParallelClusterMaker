...
if not os.path.realpath(sys.executable).startswith(os.path.realpath(os.path.join(_repo_root, '.venv'))):
    sys.exit(
...
def _validate_network(ec2client, az, vpc_name, headnode_subnet_id,
                      compute_az_list, compute_subnet_ids_override,
    ...
    def _discover_subnet(target_az, private_only=False):
        ...
        filters = [{'Name': 'availabilityZone', 'Values': [target_az]},
        ...
        if not subnets:
            suffix = ' (private subnets only — map-public-ip-on-launch=false)' if private_only else ''
            refer_to_docs_and_quit(f'No subnets found in AZ {target_az} within VPC {vpc_id}{suffix}.')
        if len(subnets) > 1:
            print(f"  WARNING: {len(subnets)} subnets found in {target_az}; using {subnets[0]['SubnetId']}."
    ...
    if compute_subnet_ids_override:
        compute_subnet_ids = [s.strip() for s in compute_subnet_ids_override.split(',') if s.strip()]
        ...
        compute_subnet_ids = [_discover_subnet(caz, private_only=_private) for caz in compute_az_list]

    return vpc_id, headnode_subnet_id, compute_subnet_ids, vpc_cidr


def _setup_iam(iam, ec2_iam_role, ec2_iam_policy, ec2_json_policy_template,
               aws_account_id, prod_level, cluster_serial_number, cluster_name,
               cluster_owner, cluster_serial_datestamp, ec2_json_policy_src,
               region='', vpc_id=''):
    ...
    with open(os.open(ec2_json_policy_template, os.O_WRONLY | os.O_CREAT | os.O_TRUNC, 0o600), 'w') as fh:
        fh.write(policy)
    iam.create_role(
    ...
    iam.put_role_policy(RoleName=ec2_iam_role, PolicyName=ec2_iam_policy, PolicyDocument=policy)
    print(f"  Created ec2_iam_role:   {ec2_iam_role}")
    print(f"  Created ec2_iam_policy: {ec2_iam_policy}")



def _setup_fsx_hydration_iam(iam, ec2_iam_role,
                              fsx_hydration_iam_policy, fsx_hydration_json_policy_src,
                              fsx_hydration_policy_template, fsx_s3_export_bucket, fsx_s3_import_bucket):
    ...
    with open(os.open(fsx_hydration_policy_template, os.O_WRONLY | os.O_CREAT | os.O_TRUNC, 0o600), 'w') as fh:
        fh.write(policy)
    iam.put_role_policy(RoleName=ec2_iam_role, PolicyName=fsx_hydration_iam_policy, PolicyDocument=policy)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='make_pcluster.py: Command-line interface to build custom ParallelCluster stacks in AWS')
    ...
    parser.add_argument('--use_defaults', metavar='DEFAULTS_FILE',
    ...
    parser.add_argument('--ansible_verbosity', help='Set the Ansible verbosity level (default = none)', required=False, default=None)
    parser.add_argument('--base_os', choices=['ubuntu2204', 'ubuntu2404', 'rhel8', 'rhel9'], help='cluster base operating system (default = ubuntu2404)', required=False, default=None)
    parser.add_argument('--cluster_lifetime', help='cluster lifetime in days:hours:minutes (default = 7:0:0)', required=False, default=None)
    parser.add_argument('--cluster_owner_department', choices=['analytics', 'clinical', 'commercial', 'compbio', 'compchem', 'datasci', 'design', 'development', 'hpc', 'imaging', 'manufacturing', 'medical', 'modeling', 'operations', 'proteomics', 'robotics', 'qa', 'research', 'scicomp'], help='department of the cluster_owner (default = hpc)', required=False, default=None)
    parser.add_argument('--cluster_type', choices=['ondemand', 'spot'], help='ondemand or spot instances (default = spot)', required=False, default=None)
    parser.add_argument('--compute_instance_type', help='compute EC2 instance type (default = c5.2xlarge)', required=False, default=None)
    parser.add_argument('--compute_root_volume_size', help='compute EBS root volume size in GB (default = 250)', required=False, type=int, default=None)
    parser.add_argument('--compute_root_volume_type', choices=['gp2', 'gp3', 'io1', 'io2', 'st1'], help='compute root EBS volume type (default = gp3)', required=False, default=None)
    parser.add_argument('--compute_root_volume_iops', help='compute root volume IOPS for gp3/io1/io2 (default = 3000)', required=False, type=int, default=None)
    parser.add_argument('--compute_root_volume_throughput', help='compute root volume throughput in MB/s for gp3 (default = 125)', required=False, type=int, default=None)
    parser.add_argument('--custom_ami', help='custom AMI ID (default = NONE)', required=False, default=None)
    parser.add_argument('--debug_mode', '-D', choices=['true', 'false'], help='enable debug mode (default = false)', required=False, default=None)
    parser.add_argument('--ebs_encryption', choices=['true', 'false'], help='enable EBS encryption (default = false)', required=False, default=None)
    parser.add_argument('--ebs_shared_dir', help='shared EBS mount path (default = /shared)', required=False, default=None)
    parser.add_argument('--ebs_shared_volume_size', help='EBS shared volume size in GB (default = 250)', required=False, type=int, default=None)
    parser.add_argument('--ebs_shared_volume_type', choices=['gp2', 'gp3', 'io1', 'io2', 'st1'], help='EBS volume type (default = gp3)', required=False, default=None)
    parser.add_argument('--ebs_shared_volume_iops', help='EBS shared volume IOPS, applies to gp3/io1/io2 (default = 3000)', required=False, type=int, default=None)
    parser.add_argument('--ebs_shared_volume_throughput', help='EBS shared volume throughput in MB/s, applies to gp3 (default = 125)', required=False, type=int, default=None)
    parser.add_argument('--efs_encryption', choices=['true', 'false'], help='enable EFS encryption in transit (default = false)', required=False, default=None)
    parser.add_argument('--efs_performance_mode', choices=['generalPurpose', 'maxIO'], help='EFS performance mode (default = generalPurpose)', required=False, default=None)
    parser.add_argument('--efs_throughput_mode', choices=['bursting', 'provisioned', 'elastic'], help='EFS throughput mode (default = bursting)', required=False, default=None)
    parser.add_argument('--enable_efs', choices=['true', 'false'], help='enable EFS (default = false)', required=False, default=None)
    parser.add_argument('--enable_efa', choices=['true', 'false'], help='enable EFA (default = false)', required=False, default=None)
    parser.add_argument('--enable_external_nfs', choices=['true', 'false'], help='enable external NFS mounts (default = false)', required=False, default=None)
    parser.add_argument('--enable_fsx', choices=['true', 'false'], help='enable FSx for Lustre (default = false)', required=False, default=None)
    parser.add_argument('--enable_ganglia', choices=['true', 'false'], help='enable Ganglia monitoring (default = false)', required=False, default=None)
    parser.add_argument('--enable_hpc_performance_tests', choices=['true', 'false'], help='deploy HPC performance test suite (default = false)', required=False, default=None)
    parser.add_argument('--external_nfs_server', help='external NFS server hostname (default = none)', required=False, default=None)
    parser.add_argument('--enable_fsx_hydration', choices=['true', 'false'], help='enable FSxL hydration from S3 (default = false)', required=False, default=None)
    parser.add_argument('--fsx_s3_import_bucket', help='S3 bucket to hydrate Lustre from (default = UNDEFINED)', required=False, default=None)
    parser.add_argument('--fsx_s3_import_path', help='S3 import path (default = import)', required=False, default=None)
    parser.add_argument('--fsx_s3_export_bucket', help='S3 bucket to dehydrate Lustre to (default = UNDEFINED)', required=False, default=None)
    parser.add_argument('--fsx_s3_export_path', help='S3 export path (default = export)', required=False, default=None)
    parser.add_argument('--fsx_size', help='Lustre file system size in GB, multiples of 1200 (default = 1200)', required=False, type=int, default=None)
    parser.add_argument('--fsx_chunk_size', help='S3 import chunk size in MB (default = 1024)', required=False, type=int, default=None)
    parser.add_argument('--hyperthreading', choices=['true', 'false'], help='enable Intel HyperThreading (default = true)', required=False, default=None)
    parser.add_argument('--initial_queue_size', help='initial compute node count (default = 2)', required=False, type=int, default=None)
    parser.add_argument('--maintain_initial_size', choices=['true', 'false'], help='keep initial nodes always running (default = false)', required=False, default=None)
    parser.add_argument('--headnode_instance_type', help='head node EC2 instance type (default = c5.xlarge)', required=False, default=None)
    parser.add_argument('--headnode_root_volume_size', help='head node EBS root volume size in GB (default = 250)', required=False, type=int, default=None)
    parser.add_argument('--headnode_root_volume_type', choices=['gp2', 'gp3', 'io1', 'io2', 'st1'], help='head node root EBS volume type (default = gp3)', required=False, default=None)
    parser.add_argument('--headnode_root_volume_iops', help='head node root volume IOPS for gp3/io1/io2 (default = 3000)', required=False, type=int, default=None)
    parser.add_argument('--headnode_root_volume_throughput', help='head node root volume throughput in MB/s for gp3 (default = 125)', required=False, type=int, default=None)
    parser.add_argument('--max_queue_size', help='maximum compute node count (default = 10)', required=False, type=int, default=None)
    parser.add_argument('--matrix_sizes', help='space-separated Axb_random matrix dimensions (default = "1000 2000 3000 4000 5000")', required=False, default=None)
    parser.add_argument('--perftest_custom_start_number', help='first custom perf job count (default = 10)', required=False, type=int, default=None)
    parser.add_argument('--perftest_custom_step_size', help='custom perf job step size (default = 10)', required=False, type=int, default=None)
    parser.add_argument('--perftest_custom_total_tests', help='number of custom perf tests (default = 5)', required=False, type=int, default=None)
    parser.add_argument('--placement_group', choices=['NONE', 'DYNAMIC'], help='EC2 placement group (default = NONE)', required=False, default=None)
    parser.add_argument('--pre_install_script', help='pre-installation script path relative to repo root (default = scripts/pre-deployment.sh)', required=False, default=None)
    parser.add_argument('--post_install_script', help='post-installation script path relative to repo root (default = scripts/post-deployment.sh)', required=False, default=None)
    parser.add_argument('--prod_level', choices=['dev', 'test', 'stage', 'prod'], help='operating level (default = dev)', required=False, default=None)
    parser.add_argument('--project_id', '-P', help='project name or ID (default = UNDEFINED)', required=False, default=None)
    parser.add_argument('--pcluster_create_timeout', help='stack creation poll retries, each 60 s (default = 60 → 60 min)', required=False, type=int, default=None)
    parser.add_argument('--scaledown_idletime', help='idle minutes before compute node terminates (default = 5)', required=False, type=int, default=None)
    parser.add_argument('--scheduler', '-S', choices=['slurm'], help='cluster scheduler (default = slurm)', required=False, default=None)
    parser.add_argument('--turbot_account', '-T', help='Turbot account ID, set to "disabled" if not used (default = disabled)', required=False, default=None)
    parser.add_argument('--vpc_name', help='VPC Name tag (default = vpc_default)', required=False, default=None)
    ...
    parser.add_argument('--headnode_subnet_id', help='explicit subnet ID for the head node; overrides auto-discovery', required=False, default=None)
    parser.add_argument('--compute_az', help='comma-separated AZs for the compute fleet (default: same as --az)', required=False, default=None)
    parser.add_argument('--compute_subnet_ids', help='comma-separated subnet IDs for compute nodes; overrides auto-discovery', required=False, default=None)
    parser.add_argument('--use_private_compute_subnet', choices=['true', 'false'], help='deploy compute nodes into private subnets (default = false)', required=False, default=None)
    ...
    cluster_lifetime         = _resolve('cluster_lifetime')
    ...
    for _script_name, _script_val in (('pre_install_script', pre_install_script),
    ...
    compute_az_list = [a.strip() for a in compute_az_raw.split(',') if a.strip()] if compute_az_raw else [az]
    ...
    try:
        ...
        ANSIBLE_VERSION = _av.stdout.splitlines()[0].split()[-1].rstrip(']') if _av.returncode == 0 else ''
    ...
    try:
        ...
        _az_info = ec2client.describe_availability_zones(ZoneNames=[az])
            ...
            botocore.exceptions.ClientError) as _e:
    ...
    if os.path.isfile(vars_file_path):
        ...
        print('$ ./kill_pcluster.py -N ' + cluster_name + ' -O ' + cluster_owner + ' -A ' + az)
    ...
    with ThreadPoolExecutor(max_workers=3) as _pool:
        _fut_network  = _pool.submit(_validate_network, ec2client, az, vpc_name,
                                     headnode_subnet_id, compute_az_list,
                                     compute_subnet_ids_override, use_private_compute_subnet)
    ...
    try:
        ...
        sys.exit("ERROR: 'pcluster' command not found. Install aws-parallelcluster before running this script.")
    except Exception as _e:
    ...
    cluster_data_dir = os.path.join(_repo_root, 'active_clusters', cluster_name) + os.sep
    ...
    cluster_serial_number_file, cluster_serial_number, cluster_serial_datestamp, _serial_was_created = \
    ...
    _default_head_node_instance_type = default_instance_types['default_head_node_instance_type']
    _default_compute_instance_type   = default_instance_types['default_compute_instance_type']
    ...
    if enable_efa:
        if compute_instance_type not in ec2_instances_efa:
            print(f'WARNING: {compute_instance_type} is not in the known EFA-capable instance list.')
    ...
    if enable_fsx:
        if (not enable_fsx_hydration) and ((fsx_s3_import_bucket != 'UNDEFINED') or (fsx_s3_export_bucket != 'UNDEFINED')):
    ...
    if enable_fsx and enable_fsx_hydration:
        fsx_s3_export_bucket, fsx_s3_export_path = _normalize_fsx_buckets(
            fsx_s3_import_bucket, fsx_s3_export_bucket,
            fsx_s3_import_path, fsx_s3_export_path,
        )
    ...
    if int(headnode_root_volume_size) > 16384 or int(compute_root_volume_size) > 16384 or int(ebs_shared_volume_size) > 16384:
        error_msg = (
    ...
    # without paying close to on-demand. Override by passing --spot_buffer if needed.
    spot_buffer = 0.5
    raw_spot_price = 0.0
    ...
    ec2_json_policy_src     = os.path.join(_repo_root, 'templates', 'ParallelClusterInstancePolicy.json_src')
    ec2_json_policy_template = os.path.join(cluster_data_dir, 'ParallelClusterInstancePolicy.json')

    _setup_iam(iam, ec2_iam_role, ec2_iam_policy, ec2_json_policy_template,
               aws_account_id, prod_level, cluster_serial_number, cluster_name,
               cluster_owner, cluster_serial_datestamp, ec2_json_policy_src,
    ...
    if enable_fsx_hydration:
        ...
        fsx_hydration_json_policy_src = os.path.join(_repo_root, 'templates', 'LustreS3HydrationPolicy.json_src')
        fsx_hydration_policy_template = os.path.join(cluster_data_dir, 'LustreS3HydrationPolicy.json')
        _setup_fsx_hydration_iam(iam, ec2_iam_role,
                                 fsx_hydration_iam_policy, fsx_hydration_json_policy_src,
                                 fsx_hydration_policy_template, fsx_s3_export_bucket, fsx_s3_import_bucket)
    else:
    ...
    #
    local_workingdir = os.getcwd()

    cluster_parameters = {
        'local_workingdir': local_workingdir,
    ...
    _jenv = Environment(loader=_FSLoader(os.path.join(_repo_root, 'templates')), keep_trailing_newline=True, undefined=StrictUndefined)
    ...
    _ansible_cmd = ['ansible-playbook', '--extra-vars', _extra_vars_str, _create_playbook]
    ...
    if debug_mode:
        ...
        abort_timer = 5
    ctrlC_Abort(abort_timer, line_length, vars_file_path, cluster_serial_number_file, cluster_serial_number, _b(enable_fsx_hydration))
    ...
    if _build_result.returncode != 0:
        ...
        if enable_fsx_hydration:
            try:
                ...
                print(f'  Warning: could not delete FSx policy {fsx_hydration_iam_policy}: {_e}')
        try:
