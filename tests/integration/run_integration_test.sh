...
# An array, not a string. As a string this needed to word-split on every use, so
# every `ssh $SSH_OPTS` was an unquoted expansion -- and shellcheck's own SC2086
# fix (`"$SSH_OPTS"`) is wrong here: it hands ssh one 90-character argument and
# every call fails. The array splits on element boundaries instead, so the key
# path is quoted while the options stay separate arguments.
SSH_OPTS=(
...
JOB_ID="$(ssh "${SSH_OPTS[@]}" "${EC2_USER}@${HEAD_IP}" \
    "sbatch --wrap 'sleep 5 && echo INTEGRATION_TEST_OK' --output /tmp/itest-job.out -p compute \
    | awk '{print \$NF}'")"
...
for i in $(seq 1 10); do
    # shellcheck disable=SC2029  # ${JOB_ID} must expand here: it exists only on
    # this side, and sacct needs the literal id.
