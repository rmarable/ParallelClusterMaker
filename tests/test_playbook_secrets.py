...
generic cluster_data_dir -> S3 sync, and that individual task parameters the
build depends on (download checksums, SSM parameter paths, feature gates)
...
# The teardown flag-plumbing checks that used to live here (kill_pcluster.py's
# extra-vars block reaching delete_pcluster.yml, and vice versa) tested a path
# that no longer exists: core_delete_cluster now tears down clusters via
# boto3/pcluster.lib and never invokes delete_pcluster.yml at all. The
# playbook itself stays in the repo as the reference spec the Python port was
# built to replicate (see pcluster_core.py's teardown functions and their
# docstrings); the two internal-consistency checks above it in this file
# (benchmark/monitoring gating) still apply to it on that basis and are kept.
