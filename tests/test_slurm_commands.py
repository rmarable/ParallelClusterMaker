...
class TestTheReadOnlyToolCannotWrite:
    ...
    def test_scontrol_show_is_emitted_not_inferred(self):
        ...
        argv, err = _validate_slurm_command("scontrol", "nodes", None, allow_writes=False)
    ...
    def test_the_cluster_redirect_is_refused(self, switches):
        ...
        tool was called for."""
        argv, err = _validate_slurm_command("scontrol", switches, None, allow_writes=False)
...
class TestTheWriteToolBoundsWhatItCan:
    ...
    def test_bulk_cancel_filters_are_refused(self, switches):
        ...
        argv, err = _validate_slurm_command("scancel", switches, None, allow_writes=True)
    ...
    def test_scancel_by_id_works(self):
        argv, err = _validate_slurm_command("scancel", "12345", None, allow_writes=True)
...
class TestTheSbatchPayloadIsCorrectShell:
    ...
    def _run(self, payload, fake_sbatch_dir):
        return subprocess.run(
            ...
            text=True,
            env=dict(
                os.environ, PATH=f"{fake_sbatch_dir}:{os.environ['PATH']}", HOME=fake_sbatch_dir
            ),
        )
    ...
    def test_a_script_containing_every_shell_metacharacter_round_trips(self, fake_sbatch):
    ...
    def test_the_temp_file_is_removed_even_when_sbatch_fails(self, tmp_path):
        ...
        r = subprocess.run(
            ...
            capture_output=True,
            text=True,
            env=dict(os.environ, PATH=f"{tmp_path}:{os.environ['PATH']}", HOME=str(tmp_path)),
        )
...
class TestTheConnectionIsNotReimplemented:
    ...
    def _call(self, monkeypatch, tmp_path, **kw):
        ...
        monkeypatch.setattr("pcluster_core.core_ensure_generated_script", lambda **k: None)
        monkeypatch.setattr(
            "pcluster_core._resolve_access_script_path",
            lambda root, name: str(tmp_path / "access.sh"),
        )
        res = core_run_slurm_command(
            cluster_data_root=str(tmp_path),
            cluster_name="c",
            cluster_record=self._REC,
            repo_root=REPO_ROOT,
            runner=_runner,
            **kw,
        )
    ...
    def test_it_invokes_the_generated_script(self, monkeypatch, tmp_path):
        res, seen = self._call(
            monkeypatch, tmp_path, command="sinfo", switches="-N", allow_writes=False
        )
    ...
    def test_the_node_type_reaches_the_script(self, monkeypatch, tmp_path):
        ...
        res, seen = self._call(monkeypatch, tmp_path, command="sinfo", allow_writes=False)
        assert seen["env"]["ACCESS_NODE_TYPE"] == "LoginNode"
        assert res.node_type == "LoginNode"

    def test_a_cluster_with_no_login_pool_uses_the_head_node(self, monkeypatch, tmp_path):
        rec = dict(self._REC, enable_loginnode="false", loginnode_count=0)
        monkeypatch.setattr("pcluster_core.core_ensure_generated_script", lambda **k: None)
        monkeypatch.setattr(
            "pcluster_core._resolve_access_script_path", lambda root, name: str(tmp_path / "a.sh")
        )
        res = core_run_slurm_command(
            cluster_data_root=str(tmp_path),
            cluster_name="c",
    ...
    def test_a_metacharacter_is_transmitted_as_data_not_executed(self, monkeypatch, tmp_path):
...
class TestWhatTheCallerIsTold:
    ...
    def test_a_slurm_rejection_is_a_result_not_an_exception(self, monkeypatch, tmp_path):
    ...
    def test_a_non_sbatch_command_has_no_job_id(self, monkeypatch, tmp_path):
        res = self._run(
            monkeypatch, tmp_path, lambda a, e, t: (0, "x", ""), command="sinfo", allow_writes=False
        )
    ...
    def test_an_invalid_command_raises_rather_than_returning(self, monkeypatch, tmp_path):
        with pytest.raises(PClusterMakerError):
            self._run(
                monkeypatch, tmp_path, lambda a, e, t: (0, "", ""), command="rm", allow_writes=True
            )
...
class TestStdinIsClosed:
    ...
    def test_the_real_runner_closes_stdin(self):
        ...
        rc, out, err = _run_access_script(["bash", "-c", "cat; echo done"], dict(os.environ), 10)
        assert rc == 0
...
class TestTheAccessScriptKeepsStdoutClean:
    def test_no_progress_message_goes_to_stdout(self):
        ...
        src = open(os.path.join(REPO_ROOT, "templates", "access_cluster.j2")).read()
...
class TestTheSSMDocumentIsTheBoundary:
    ...
    def _doc():
        ...
        return json.load(open(os.path.join(REPO_ROOT, "templates", "ssm_slurm_readonly.json")))
...
class TestTheRemotePathIsReadOnlyByConstruction:
    ...
    def test_the_iam_names_the_document_not_the_shell_runner(self):
        ...
        d = json.loads(
            open(os.path.join(REPO_ROOT, "templates", "MCPReadOnlyLambda.json_src"))
            .read()
        ...
        assert docs == ["arn:aws:ssm:*:123456789012:document/PClusterMakerSlurmReadOnly"], docs
    ...
    def test_the_remote_path_refuses_a_write_command(self, monkeypatch):
        ...
        with pytest.raises(PClusterMakerError):
            core_run_slurm_command_via_ssm(
                cluster_record={"region": "us-east-1"}, cluster_name="c", command="sbatch"
            )
    ...
    def test_truncation_is_reported_not_hidden(self, monkeypatch):
        ...
        monkeypatch.setattr(pc, "run_slurm_via_ssm", lambda **k: (0, "x" * 24000, "", True))
...
class TestTheArrayIsWhatMakesTheSwitchesSafe:
    ...
    def _decode_like_the_document(self, b64, tmp_path):
        ...
        return subprocess.run(
            ["bash", "-c", script, "_", b64], capture_output=True, text=True, cwd=tmp_path
        )

    def test_a_pipe_survives_as_data(self, tmp_path):
        from pcluster_core import _encode_slurm_switches

        r = self._decode_like_the_document(_encode_slurm_switches("-o %P|%a|%D"), tmp_path)
        assert r.returncode == 0, r.stderr
    ...
    def test_a_command_separator_is_not_executed(self, tmp_path):
        ...
        r = self._decode_like_the_document(_encode_slurm_switches(f"-N ; touch {marker}"), tmp_path)
        assert r.returncode == 0, r.stderr
    ...
    def test_empty_switches_do_not_break_under_set_u(self, tmp_path):
        ...
        r = subprocess.run(
            ["bash", "-c", script, "_", _encode_slurm_switches("")], capture_output=True, text=True
        )
        assert r.returncode == 0, r.stderr
...
class TestTheAssembledRemoteCommandActuallyRuns:
    ...
    def _execute(self, remote_cmd, cwd, extra_path):
        ...
        return subprocess.run(
            ...
            text=True,
            cwd=cwd,
            env=dict(os.environ, PATH=f"{extra_path}:{os.environ['PATH']}", HOME=str(cwd)),
        )
    ...
    def test_the_end_to_end_path_uses_the_compound_form(self):
        ...
        fn = next(
            ...
            if isinstance(n, ast.FunctionDef) and n.name == "core_run_slurm_command"
        )
        calls = [
            n
