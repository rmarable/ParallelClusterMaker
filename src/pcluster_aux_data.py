...
def ctrlC_Abort(
    ...
    try:
        ...
        if cluster_serial_number is None:
            ...
            import boto3
            from pcluster_core import _delete_managed_policies
            ...
            if not aws_account_id:
                print("WARNING: aws_account_id not available — cannot clean up managed policies")
            else:
                fsx_policy = None
...
ARM_OSES = ("ubuntu2204arm", "ubuntu2404arm", "rhel9arm", "alinux2023arm")
X86_OSES = ("ubuntu2204", "ubuntu2404", "rhel9", "alinux2023")


def is_arm_instance(instance_type):
    """Return True if instance_type is an ARM/Graviton family."""
    return any(instance_type.startswith(f) for f in ARM_FAMILIES)


def base_os_instance_check(base_os, instance_type, debug_mode):
    _arm_oses = ARM_OSES
    _x86_oses = X86_OSES
    _is_arm = is_arm_instance(instance_type)
    if _is_arm and base_os in _x86_oses:
        if debug_mode:
            print(
                f"WARNING: {instance_type} is ARM/Graviton but base_os={base_os} is x86_64 — cluster will fail to boot."
            )
        refer_to_docs_and_quit(
            f"Instance type {instance_type} is ARM/Graviton but base_os={base_os} is x86_64.\n"
            f"  Use an ARM-compatible base OS (e.g. {base_os}arm) or an x86_64 instance type."
        )
    if (not _is_arm) and base_os in _arm_oses:
        if debug_mode:
            print(
                f"WARNING: {instance_type} is x86_64 but base_os={base_os} is ARM/Graviton — cluster will fail to boot."
            )
        refer_to_docs_and_quit(
            f"Instance type {instance_type} is x86_64 but base_os={base_os} is ARM/Graviton.\n"
            f"  Use an x86_64 base OS (e.g. {base_os.removesuffix('arm')}) or an ARM/Graviton instance type."
        )
    if debug_mode:
...
def nvidia_gpu_count(instance_type_info):
    """NVIDIA GPU count from one describe_instance_types InstanceTypes entry."""
    # NVIDIA only: Slurm GRES and the CUDA runtime do not see AMD (g4ad) or
    # Habana accelerators, so counting them would oversubscribe a job's ranks
    # against devices no benchmark can address.
    return sum(
        gpu.get("Count", 0)
        for gpu in (instance_type_info.get("GpuInfo") or {}).get("Gpus", [])
        if gpu.get("Manufacturer", "").upper() == "NVIDIA"
    )


def usable_vcpu_count(instance_type_info, *, hyperthreading):
    """Schedulable vCPUs from one describe_instance_types InstanceTypes entry."""
    # DisableSimultaneousMultithreading is what config.pcluster.j2 sets when
    # hyperthreading is false, and upstream only acts on it when the instance
    # reports more than one thread per core (`disable_simultaneous_multithreading
    # and self.instance_type_info.default_threads_per_core() > 1`,
    # pcluster/config/cluster_config.py:1523). Graviton reports 1, so dividing by
    # a hardcoded 2 would request half the cores every ARM node actually has --
    # the `or 1` below is what covers both that case and a missing field, and it
    # makes the division an identity rather than a special case.
    vcpu_info = instance_type_info.get("VCpuInfo") or {}
    vcpus = vcpu_info.get("DefaultVCpus") or 0
    if hyperthreading:
        return vcpus
    return vcpus // (vcpu_info.get("DefaultThreadsPerCore") or 1)


def derive_ranks_per_node(*, instance_types, vcpu_map):
    """Schedulable rank count every node in one queue can satisfy, or 0 if empty."""
    # A queue may hold several instance types, so the minimum is the only count
    # that fits on all of them. Floor of 1 for a queue that exists: `sbatch
    # --ntasks=0` is rejected outright, so a type missing from vcpu_map -- or one
    # EC2 reports with no VCpuInfo -- must not render an unsubmittable script.
    if not instance_types:
        return 0
    return max(1, min(vcpu_map.get(t, 0) for t in instance_types))


def parse_instance_type_list(raw):
    """Split comma-separated instance type string, deduplicate, preserve order."""
    if not raw:
        return []
    seen = set()
    result = []
