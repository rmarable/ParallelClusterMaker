...
def _validate_cluster_name(name):
    ...
    """
    if (not re.match(r'^[a-z]([a-z0-9\-]{0,25}[a-z0-9])?$', name)
            or '--' in name):
        sys.exit(
...
def _validate_cluster_owner(owner):
    ...
    """
    if (not re.match(r'^[a-z0-9][a-z0-9\-]{0,62}$', owner)
            or owner.endswith('-')
            or '--' in owner):
        sys.exit(
...
def _load_or_create_serial(cluster_data_dir, cluster_name):
    ...
    if os.path.isfile(serial_file):
        ...
        if not serial_number:
            sys.exit(f"ERROR: Serial file {serial_file} is empty or corrupted. Delete it and re-run to start fresh.")
        ...
        with open(os.open(serial_file, os.O_WRONLY | os.O_CREAT | os.O_TRUNC, 0o600), 'w') as fh:
...
def _check_fsx_s3(s3_client, bucket, path, label):
    ...
    try:
        ...
        code = _e.response.get('Error', {}).get('Code', '') if hasattr(_e, 'response') and _e.response else ''
        if code == '403':
            sys.exit(f"ERROR: Lustre hydration: {label} bucket s3://{bucket} exists but access is denied — check bucket policy and IAM role.")
...
def _resolve_bool(name, args, file_defaults, hardcoded_defaults):
    ...
    if val is None:
        sys.exit(f"ERROR: required boolean parameter '{name}' has no value in CLI args, defaults file, or hardcoded defaults.")
...
def _resolve_access_script_path(cluster_data_root, cluster_name):
    ...
    path = os.path.normpath(os.path.join(
        root, cluster_name,
        f'access_cluster.{cluster_name}.sh'
    ))
