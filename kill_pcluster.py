...
def main():
    ...
    parser.add_argument('--ansible_verbosity', '-V', help='Ansible verbosity level (default = none)', required=False, default=None)
    parser.add_argument('--delete_fsx', choices=['true', 'false'], help='delete FSx on cluster teardown (default = true)', required=False, default=None)
    parser.add_argument('--delete_s3_bucketname', choices=['true', 'false'], help='delete S3 bucket on cluster teardown (default = true)', required=False, default=None)
    ...
    if not _az_info.get('AvailabilityZones'):
        refer_to_docs_and_quit(f'"{az}" is not a valid Availability Zone in the selected AWS Region.')
    ...
    _active_clusters_root = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'active_clusters')
    ...
    _ansible_cmd = ['ansible-playbook', '--extra-vars', _destroy_extra_vars_str, _delete_playbook]
    ...
    try:
        ...
        print('Check the output above and the CloudFormation console before assuming the cluster is gone.')
