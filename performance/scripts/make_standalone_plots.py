...
matplotlib.use(
    "Agg"
)  # must precede pyplot import; Agg works on all headless envs including EC2
...
if not _summary_dir.is_dir():
    sys.exit(
        f"ERROR: summary_final/ not found at {_summary_dir}\n"
...
print("")
print(
    "Generating graphs of the CSV data files generated from run_axb.sh / hpc-perftest.sh..."
)
...
for datafile in glob.iglob(str(_summary_dir / "*.csv")):
    ...
    if args.plot == "unified":
        ...
        _fit_curve(
            ax, df["total_compute_time"], df["task_array_size"], line.get_color()
        )
...
if not y_axis_max:
    sys.exit(
        f"ERROR: no CSV files found in {_summary_dir}\nRun run_axb.sh / hpc-perftest.sh first."
    )
...
_matrix_label = (
    f'Matrix Sizes = [matrix.conf settings: {", ".join(MATRIX_SIZES.split())}]'
)
plt.title(titles[args.plot] + _matrix_label, weight="bold")
plt.legend(
    loc="best", fontsize="small", markerscale=0.8, title="DataSource_InstanceType"
)
...
if x_axis_min and x_axis_max:
    ...
    plt.gca().xaxis.set_major_locator(
        mpl.ticker.MaxNLocator(nbins=10, steps=[1, 2, 5, 10])
    )
...
if y_axis_min and y_axis_max:
    ...
    plt.gca().yaxis.set_major_locator(
        mpl.ticker.MaxNLocator(nbins=10, integer=True, steps=[1, 2, 5, 10])
    )
