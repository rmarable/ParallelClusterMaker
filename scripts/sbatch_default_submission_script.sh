...
# Select the partition and the number of compute cores.
#
# Both values are derived at build time from what this cluster actually has.
# The rank count is EC2's own DefaultVCpus for the smallest instance type in the
# partition -- divided by DefaultThreadsPerCore when hyperthreading is disabled,
# which is what DisableSimultaneousMultithreading does to the node -- so it is a
# count every node in the queue can satisfy.
{% if enable_cpu_queue == 'true' %}
# Partition "compute": {{ cpu_instance_types | join(', ') }}
# Hyperthreading is {{ 'enabled' if hyperthreading == 'true' else 'disabled' }}.
#SBATCH --partition=compute
#SBATCH --ntasks={{ cpu_ranks_per_node }}
{% else %}
# This cluster has no CPU queue, so "gpu" is its only partition.  The rank
# count is still a core count, not a GPU count -- this is a general-purpose
# job script, not the GPU benchmark driver.
# GPU nodes: {{ gpu_instance_types | join(', ') }}
# Hyperthreading is {{ 'enabled' if hyperthreading == 'true' else 'disabled' }}.
#SBATCH --partition=gpu
#SBATCH --ntasks={{ gpu_vcpus_per_node }}
{% endif %}
