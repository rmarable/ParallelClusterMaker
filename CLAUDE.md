...
- **IAM policy is split into two inline policies.** `ParallelClusterInstancePolicy.json_src` no longer exists. The policy is stored as `ParallelClusterInstancePolicy-A.json_src` (33 statements, ≤9,468 bytes minified) and `-B.json_src` (7 statements, ≤2,420 bytes minified) to stay under IAM's 10,240-byte inline policy limit. Both policies are named `<ec2_iam_policy>-A` and `-B` and must be deleted together on teardown.
